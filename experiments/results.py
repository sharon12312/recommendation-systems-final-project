...
class Results:
    ...
    def save(self, hyperparams, test_mrr, validation_mrr, elapsed):
        ...
        result.update({'test_mrr': test_mrr, 'validation_mrr': validation_mrr, 'elapsed': elapsed, 'hash': self._hash(hyperparams)})

        with open(self._filename, 'a+') as out:
            out.write(json.dumps(result) + '\n')

    def best_baseline(self):
        baseline_results = [x for x in self if x['compression_ratio'] == 1.0 and x['embedding_dim'] >= 32]
